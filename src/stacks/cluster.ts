  ...
  constructor(scope: Construct, id: string, props: ClusterStackProps) {
    ...
    this.clusterRoles = new ClusterRoles(this, "ClusterRoles", {
      nodeRolePolicyStatement:
        props.optionalClusterStackProps?.nodeRolePolicyStatement,
     ...
     */
    this.vpc = props.vpc;
    this.clusterSecurityGroups = new ClusterSecurityGroups(
      this,
      "ClusterNetworkPrep",
      { vpc: props.vpc }
    );
     ...
     */
    const roleMappings: { [id: string]: string } =
      props.optionalClusterStackProps?.roleMappings || {};
    for (let mappingKey in roleMappings) {
      let value: string = roleMappings[mappingKey];
      let discoveredRole = Role.fromRoleName(
        this,
        "AwsAuth-" + mappingKey,
        mappingKey,
        { mutable: false }
