...
export interface ClusterRolesProps {
  /**
  ...
  constructor(scope: cdk.Stack, id: string, props?: ClusterRolesProps) {
    ...
    this.clusterRole = new Role(this, "ClusterRole", {
      description: cdk.Fn.join("/", [
        scope.stackName,
        "Minimal EKS CdkCluster Service Role",
      ]),
      assumedBy: new CompositePrincipal(
    ...
    if (props?.existingMasterRole) {
      this.masterRole = Role.fromRoleName(
        this,
        "ExistingMasterRole",
        props.existingMasterRole,
        {
          mutable: false,
        }
      );
      ...
      this.masterRole = new Role(this, "MasterRole", {
        description: cdk.Fn.join("/", [
          scope.stackName,
          "Minimal EKS Master Access Role",
        ]),
