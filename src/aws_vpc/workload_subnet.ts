...
export class WorkloadSubnets extends Construct {
  ...
  constructor(scope: Stack, id: string, props: WorkloadSubnetProps) {
    ...
    this.blockSize =
      props.optionalWorkloadSubnetProps?.blockSize ||
      DEFAULT_WORKLOAD_BLOCKSIZE;
    this.cidr =
      props.optionalWorkloadSubnetProps?.cidr || DEFAULT_WORKLOAD_CIDR;
    this.subnets = [];
    this.subnetIds = [];
    ...
    props.vpc.privateSubnets.forEach((existingPrivate, i) => {
      ...
      let subnet = new CfnSubnet(this, `Subnet-${i}`, {
        ...
        cidrBlock: Fn.select(
          ...
          Fn.cidr(
            this.cidr,
            props.vpc.availabilityZones.length,
            cidrSize.toString()
          )
        ),
      });
      ...
      let routeAssoc = new CfnSubnetRouteTableAssociation(
        this,
      ...
      this.subnets.push(
        Subnet.fromSubnetId(this, `SubnetLookup-${i}`, subnet.ref)
      );
